...
{   
    ...
    {
        var videos = await dbContext.VideosInfos.Where(video => video.IsUploaded).OrderBy(video =>  video.Id).Take(10).Select<VideoInfo, VideoCard>(video =>
            new VideoCard
            {
                Id = video.Id,
                Title = video.Title,
    ...
    public async Task<Results<PhysicalFileHttpResult, NotFound>> GetVideoAsync(string id, CancellationToken cancellationToken = default)
    {
        var result = await dbContext.VideosInfos.FirstOrDefaultAsync(v => v.Id == id, cancellationToken: cancellationToken);
        if (result is null || !result.IsUploaded)
        {
            return TypedResults.NotFound();
        }
        logger.LogInformation("Sent video by id: {id}", id);
        return TypedResults.PhysicalFile(Path.Combine(FilesPath, result.Id, "video.mp4"));
    }

    [HttpGet("{id}/preview")]
    public async Task<Results<PhysicalFileHttpResult, NotFound>> GetPreviewAsync(string id, CancellationToken cancellationToken = default)
    {
        var result = await dbContext.VideosInfos.FirstOrDefaultAsync(v => v.Id == id, cancellationToken: cancellationToken);
        if (result is null || !result.IsUploaded)
        {
        ...
        return TypedResults.PhysicalFile(Path.Combine(FilesPath, result.Id, "preview.jpg"));
    }

    [HttpPost]
    public async Task<Ok<string>> PostVideoMetadataAsync([FromForm] PostFileMetadata postFileMetadata, CancellationToken cancellationToken = default)
    {
    ...
    [HttpPost("{id}")]
    public async Task<Results<Ok<string>, NotFound<string>>> PostVideoAsync([FromForm] IFormFile formFile, string id, CancellationToken cancellationToken = default)
    {
